	...
	for (let i = 0; i < expression.length; ++i) {
		if (
			...
			if (expression[i] === '+' || expression[i] === '-') {
				if (
					!i ||
					precedence(expression[i - 1]) ||
					expression[i - 1] === '('
				) {
					cache += '0';
				}
			}
			if (cache.length) expressionAsList.push(cache);
			cache = '';
...
let infixToPostfix = (asList) => {
	...
	for (let i = 0; i < asList.length; ++i) {
		if (!isNaN(asList[i])) postfixList.push(asList[i]);
		else if (asList[i] === '(') {
			if (i && !precedence(asList[i - 1]) && asList[i - 1] != '(') {
				while (
					stack.length > 0 &&
					precedence('×') < precedence(stack[stack.length - 1])
				) {
			...
			stack.pop();
		} else if (asList[i] === '^') stack.push(asList[i]);
		else {
			while (
				stack.length > 0 &&
				precedence(asList[i]) < precedence(stack[stack.length - 1])
			) {
				postfixList.push(stack.pop());
			}
			stack.push(asList[i]);
...
let evaluate = (postfixList) => {
	...
	for (let i = 0; i < postfixList.length; ++i) {
		if (!isNaN(postfixList[i])) stack.push(postfixList[i]);
		else {
...
function main() {
	if (cache.length) infix.push(cache);
	...
	let last = infix.pop();
	if (last <= '0' || '9' >= last) infix.push(last);
}
...
function backspace() {
	...
	for (let i = 0; i < last_added.length; ++i) {
		if (precedence(last_added[i])) {
			...
			finalText +=
				`<span class="parenthesis">` + last_added[i] + '</span>';
		} else {
...
function modeChange() {
	...
	else document.getElementById('mode-shift').innerHTML = 'Light :';
}
...
function add(toBeAdded) {
	...
	if (precedence(toBeAdded)) {
		main_expression.innerHTML +=
			`<span class="operator">` + toBeAdded + `</span>`;
	} else if (toBeAdded === '(' || toBeAdded === ')') {
		main_expression.innerHTML +=
			`<span class="parenthesis">` + toBeAdded + `</span>`;
	} else {
